...
## Objective
The goal is to develop an application that displays wind farms, their turbines, and turbine components. Each component should have a condition grade from 1-5. 
I initially assumed the 4 mentioned components, Blade, Generator, Hub, and Rotor.
...
- **...view every farm on a map with their turbines**: It would be awesome to show a map where the farms are marked on. When you zoom in you should see the positioning of every turbine inside it.
- **...have a dashboard that shows summarized info**: It's important to show summarized info in the dashboard, for example, the turbines that need attention because their components have poor condition and the map with the farm's locations.
- **...easily navigate through the app**: Great UX and UI are important to reduce cognitive overload. 
...
- **Should the inspections be made to a single component or the entire turbine?**: In terms of database relationships it's also a major factor.
...
### Name 
The name of the app will be **WindFlow**. "Wind" is self-explanatory and "Flow" is related to how wind interacts with turbines and how smooth the data flows in the app.

### Logo
![WindFlow logo](https://github.com/bgrou/technical-test/assets/61094081/bf1ba32a-5c34-4b6b-809d-232209c971e4)

## Database Design
As mentioned before, the entities that should exist in this app are **Wind Farms, Turbines, Components, Manufacturers and Inspectors**. 
After some thinking, I came up with a potential dilemma:
...
I decided that inspections should be made to a single component so each of the components can have a super detailed and precise inspection. Also decided that the inspectors could be the users of the app itself (I thought about creating an inspectors table).

## Backend
It might sound controversial but I chose to follow a monolithic architecture due to its simplicity and ease of development. If it was a production app with tons of users and data, we would need to move to a decoupled architecture.
I decided to use [Inertia.js](https://inertiajs.com) so I can achieve the performance and smooth UX of a SPA while developing in Laravel with a monolithic architecture.

I used a Controller->Service->Repository design pattern due to its separation of concerns. A repository might sound overkill because I added unnecessary complexity but I prefer to isolate the application from the ORM.
...
- Repository: Query logic and provides additional abstraction layer;
- Model: Relationship management.

**Each object's responsibilities**
...
The final version ended up being different and optimized. I focused on bringing a simple, modern, and easy-to-learn platform.


## API
Inertia is intended to build single-page apps without building an API. Trying to implement an API in the same environment is not an easy task since Inertia changes Laravel configs to force it to respond to requests in an Inertia Response.
...
- Huge data, especially in the inspections table: To handle this we should use data archiving and table partitioning.
- Scalability problems due to monolithic architecture: We would need to move to a decoupled architecture to improve scalability.
- API dependant: Since we would rely on APIs like Windy, if the API breaks we would need to move to another API provider, so we need to keep our code reusable
  
## Missing features and things I should've done
Due to time constraints, I had to forgo important elements that I would definitely incorporate in normal circumstances:
- **Windy API Integration**: Unfortunately, I couldn't implement the integration of the Windy API as planned, which would have provided essential wind speed data for precise power generation calculations. The data would've been fetched every x hours through cron jobs and stored in this database's table:
  ![image](https://github.com/bgrou/technical-test/assets/61094081/44a17910-b380-467c-af63-e20553f61d93);
...
- **Git Branching**: I didn't use feature branches for each new functionality, which would have been crucial, especially in a collaborative team environment.
- **Standalone API with Authentication**: I would create a standalone API, complete with authentication mechanisms. This would have facilitated data sharing with external partners and created the foundation for a potential transition to a decoupled architecture.
- **Microservices instead of Monolith**: Without thinking twice, I would've employed Microservices architecture to achieve full efficiency and scalability potential.
